...
/**
 * This fn accepts a `generic` type, type is determined by what is passed in. further constrained to it accepting a type that is an object with a `string` key
 *
 * An argument could be made for function arrayToDict(array: ({id: string})[]):{ [k: string]: {id: string} } {const out: { [k: string]: {id: string}} } = {};} this constrains the fn to accept an array of objects with ONLY `id` key and nothing else!
 * The same happens to the return type! When we try to access something off of the returned dictionary, we won't be able to access anything but `id` key.
 */
...
// ASK for only what is needed and RETURN what you can --> when paramterizing fucntions or types!
