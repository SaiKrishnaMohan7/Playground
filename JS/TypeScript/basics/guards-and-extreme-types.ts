...
let myAny: any = 32;
let myUnknown: unknown = "hello, unknown"; // can receive any values; diff is in when you access it; It can hold anything but you have to narrow it down to use it (API responses are a good use case example)
...
// 💡 Note return type
function isHasEmail(x: any): x is HasEmail { // We are telling tsc here that the boolean that we are returning here is an indication that the returned item is of type HasEmail
  return typeof x.name === "string" && typeof x.email === "string";
}

if (isHasEmail(myUnknown)) { // using isHasEmail like a `typeof` or `instanceof` here! Narrowing down the unknown type to HasEmail type
  // In here, myUnknown is of type HasEmail
  console.log(myUnknown.name, myUnknown.email);
}

// my most common guard; best used with filter method
function isDefined<T>(arg: T | undefined): arg is T {
  return typeof arg !== "undefined";
}

const randomStringOrUndefinedArr = ['a', 'b', 'c', undefined, 'd'];
const filteredArrWithOnlyStringsUndefinedRemoved = randomStringOrUndefinedArr.filter(isDefined)
...
bb = aa; // 🚨 yikes, let's you assign an unknown to an unknown as unknown is a top type!
...
function brandA(value: string): BrandedA {
  return (value as unknown) as BrandedA; // casting!
}
